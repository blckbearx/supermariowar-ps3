...
if (PS3_BUILD)
    FetchContent_MakeAvailable(SDL2 SDL2_image SDL2_mixer yaml-cpp)
else()
...
if (PS3_BUILD)
    if (DEFINED ENV{PS3DEV})
        set(PS3_SDK_ROOT $ENV{PS3DEV})
    else()
        set(PS3_SDK_ROOT /usr/local/ps3dev-sdl2)
    endif()
    set(PS3_PORTLIBS_DIR "${PS3_SDK_ROOT}/portlibs/ppu")

    find_path(ZLIB_INCLUDE_DIR zlib.h PATHS "${PS3_PORTLIBS_DIR}/include")
    find_library(ZLIB_LIBRARY NAMES z zlib PATHS "${PS3_PORTLIBS_DIR}/lib")
    if (NOT ZLIB_LIBRARY)
        message(FATAL_ERROR "PS3 build requested but zlib library not found in PS3 SDK at ${PS3_PORTLIBS_DIR}/lib")
    endif()
    target_link_libraries(smw_zlib INTERFACE ${ZLIB_LIBRARY})
    if (ZLIB_INCLUDE_DIR)
        target_include_directories(smw_zlib INTERFACE ${ZLIB_INCLUDE_DIR})
    endif()
else()
    if (TARGET ZLIB::ZLIB)
